...
export function chunkMarkdown(
  ...
  // Split into paragraphs on double-newline boundaries, preserving line numbers.
  const lines = content.split("\n");
  const paragraphs: { text: string; lineStart: number; lineEnd: number }[] = [];

  let paraLines: string[] = [];
  let paraStart = 1; // 1-indexed
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim() === "" && paraLines.length > 0) {
      ...
      paraLines = [];
      paraStart = i + 2; // next line, 1-indexed
    } else if (line.trim() !== "") {
      if (paraLines.length === 0) {
        paraStart = i + 1; // 1-indexed
      }
      paraLines.push(line);
    } else {
      // blank line before any content — advance start
      if (paraLines.length === 0) {
        paraStart = i + 2;
      }
    }
  ...
  // If entire content fits in one chunk, return it as-is.
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (chunkLen + addLen > targetChars && chunkParas.length > 0) {
      ...
      // Overlap: walk backwards from end of chunkParas to collect ~overlapChars
      let overlapLen = 0;
