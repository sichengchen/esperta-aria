...
| `APFLOW-005` | `boundary enforcement`           | Project run tries to use Aria memory or automation outside node policy | Access is blocked because runtime policy boundaries remain explicit                 | `workflow` | `missing` | `tests/server/project-workflow-e2e.test.ts` |
